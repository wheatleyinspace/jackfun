    ...
    <div id="game" class="game pt-pageholder pollposition-background">
		<div class="pt-page-off state-lobby pollposition-page">
			<div class="container">
				...
				<form class="pure-form">
					<button type="button" id="pollposition-startgame" class="pollposition-button button-xlarge pure-button pure-input-1"><span>Все здесь</span></button>
					<button type="button" id="pollposition-stopcountdown" class="pollposition-button button-xlarge pure-button pure-input-1"><span>Отмена</span></button>
				</form>
			</div>
		</div>
		...
		<div class="pt-page-off state-post-lobby pollposition-page">
			<div class="container">
				...
				<form class="pure-form">
					<button type="button" id="pollposition-sameplayers" class="pollposition-button button-xlarge pure-button pure-input-1 pollposition-endbuttons"><span>ТЕ ЖЕ ИГРОКИ</span></button>
					<button type="button" id="pollposition-newplayers" class="pollposition-button button-xlarge pure-button pure-input-1 pollposition-endbuttons"><span>Новые игроки</span></button>
				</form>
        ...
        <div class="pt-page-off state-upordown-sent pollposition-page">
            <div class="container">
                ...
                <p class='pollposition-text'>Спасибо за зрительский выбор!</p>
            </div>
        </div>
        ...
        <div class="pt-page-off state-waitforallpercentages pollposition-page">
            <div class="container">
                <p class='pollposition-text'>Спасибо за ответ.<br />Ждём остальных игроков.</p>
            </div>
        </div>

        <div class="pt-page-off state-waitforaudience pollposition-page">
            <div class="container">
                <p class='pollposition-text'>Спасибо за ответ.<br />Ждём ответов зрителей.</p>
            </div>
        </div>
            ...
            <div class="container">
                ...
                <div class="pollposition-range-buffer"> 
                    <form class="pure-form" id="pollposition-enterpercentage-field"> 
                        <div class=""> 
                            <button type="submit" id="pollposition-submitpercentage" class="pollposition-button"><span>Подтвердить</span></button> 
        ...
        <div class="pt-page-off state-upordowndone pollposition-page">
            <div class="container">
                <p class='pollposition-text'>Готово!</p>
            </div>
        </div>
        ...
        <div class="pt-page-off state-choosecharacter pollposition-page">
            <div class="container" align="center">
                ...
                <form class="pure-form">
                    <button type="button" id="pollposition-character-startgame" class="pollposition-button button-xlarge pure-button pure-input-1"><span>Все здесь</span></button>
                    <button type="button" id="pollposition-character-stopcountdown" class="pollposition-button button-xlarge pure-button pure-input-1"><span>Отмена</span></button>
                </form>
...
const Gx = lc.extend({
    ...
    updatePlayer() {
        ...
        if (this.percentageInterval && (clearInterval(this.percentageInterval), this.percentageInterval = null), t !== void 0 && t.character !== void 0 && ae("#pollposition-character").attr("class", `pollposition-character ${t.character.name}`).show(), n === "RoomFull") {
            kt.show(Error("Нет мест в комнате"), {
                willClose: () => {
                    window.location.reload(!0)
                }
            }), this.onResize();
        ...
        if (n === "GameLocked") {
            kt.show(Error("Игра уже началась. Подождите начала новой."), {
                willClose: () => {
                    window.location.reload(!0)
                }
            }), this.onResize();
        ...
        if (i && i === "Lobby") {
            ...
            if (n === "Lobby_ChooseCharacter") {
                this.hideCharacterLobbyButtons(), ae("#pollposition-choosecharacter-text").html("Выбери своего агента");
                let v = "";
                ...
                ae("#pollposition-choosecharacter").html(v), this.showScreen(".state-choosecharacter"), t.isAllowedToStartGame && (f === "CanStart" ? (ae("#pollposition-characterlobby-text").html("Нажми эту кнопку, когда все присоединились"), ae("#pollposition-character-startgame").show()) : f === "Countdown" && (ae("#pollposition-characterlobby-text").html("Нажми эту кнопку, чтобы отменить запуск"), ae("#pollposition-character-stopcountdown").show())), this.onResize();
                return
            }
            if (f === "PostGame") {
                t.isAllowedToStartGame ? (ae("#pollposition-post-lobby-text").html("Что выберешь?"), ae(".pollposition-endbuttons").show()) : ae("#pollposition-post-lobby-text").html("Откинься на спинку и отдохни!");
                let v = '<table class="table table-striped table-bordered"><thead><tr><th>Ранг</th><th>Игрок</th><th>Результат</th></tr></thead><tbody>';
            ...
            if (!t.isAllowedToStartGame) {
                ae("#pollposition-lobby-text").html("Откинься на спинку и отдохни!"), this.showScreen(".state-lobby");
                return
            }
            f === "WaitingForMore" ? ae("#pollposition-lobby-text").html("Ожидаем других игроков...") : f === "CanStart" ? (ae("#pollposition-lobby-text").html("Нажми эту кнопку, когда все присоединились"), ae("#pollposition-startgame").show()) : f === "Countdown" && (ae("#pollposition-lobby-text").html("Нажми эту кнопку, чтобы отменить запуск"), ae("#pollposition-stopcountdown").show()), this.showScreen(".state-lobby")
        } else if (i === "Gameplay_Logo") this.showLogo();
        else if (i === "Gameplay_PickCategory") {
            if (n === "Gameplay_WaitForCategory") ae("#waitforcategory-text").html(`Ждём, пока ${e.player} выберет категорию.`), this.showScreen(".state-waitforcategory");
            else if (n === "Gameplay_PickCategory") {
                ae("#choosecategory-text").html("Выбери категорию");
                let f = "";
        ...
        }, this.percentageDelayTime), this.showScreen(".state-enterpercentage"), this.currentRange.resize()) : n === "Gameplay_WaitForPercentage" ? (this.activeScreen !== ".state-waitforpercentage" && (ae(".state-waitforpercentage .question-text").html(e.question), ae("#waitingforpercentage-text").html(`Ждём, пока ${e.player} укажет процент.`)), this.showScreen(".state-waitforpercentage")) : n === "Gameplay_WaitForAllPercentages" ? this.showScreen(".state-waitforallpercentages") : n === "Gameplay_WaitForAudience" ? this.showScreen(".state-waitforaudience") : this.showLogo();
            ...
            if (n === "Gameplay_WaitForChooseUpOrDown") this.showScreen(".state-waitforupordown");
                ...
                ae(".state-chooseupordown .survey-text").html(e.survey), ae(".question-text").html(e.question), ae("#chooseupordown-text").html("Как ты думаешь, правильный ответ...");
                let f = "";
                for (o = 0; o < e.choices.length; o++) f += `<button type='button' data-choice='${e.choices[o].id.toString()}' class='pollposition-button pollposition-high-low-button pollposition-choice-button'><span>${e.choices[o].id.toString().replace("Much_Higher","Ещё выше").replace("Much_Lower","Ещё ниже").replace("Higher","Выше").replace("Lower","Ниже").replace("_", " ")}</span></button>`;
    ...
    updateAudience() {
        ...
        if (e === "Gameplay_EnterPercentage") {
            ...
            o += "<button type='button' data-choice='-1' class='pollposition-button pollposition-choice-button'><span>Это ко мне не относится</span></button>", ae("#pollposition-audiencechooseoption").html(o), this.showScreen(".state-audience-chose-option")
            ...
            let o = '<table class="table table-striped table-bordered"><thead><tr><th>Ранг</th><th>Игрок</th><th>Результат</th></tr></thead><tbody>';
        ...
        } else ae(".state-audience-wait .audience-wait-text").html("Ты зритель! Подожди голосования."), ae(".state-audience-wait .audience-usage-text").html("Если ответят хотя бы 5 зрителей, игроки будут угадывать их ответы."), this.showScreen(".state-audience-wait")
    },
    hideLobbyButtons() {
    ...
    submitPercentage() {
        ...
        if (!this.isInt(t)) {
            ...
            return ae("#pollposition-submit-alert").html("Ты не можешь ввести пустоту!"), n.removeClass("alert-info"), n.addClass("alert-danger"), n.show(), !1
        }
    ...
    submitChoice(t) {
        ...
        return ae(".pollposition-choice-button").attr("disabled", !0), this.client.isRole("player") ? this.client.send("SendMessageToRoomOwner", {
            ...
            type: "vote",
            vote: e
        }), ae("#pollposition-audiencechooseoption").html(""), ae(".state-audience-wait .audience-wait-text").html("Спасибо, зритель!"), ae(".state-audience-wait .audience-usage-text").html("Если ответят хотя бы 5 зрителей, игроки будут угадывать их ответы."), this.showScreen(".state-audience-wait")), !1
    },
    submitCategory(t) {
