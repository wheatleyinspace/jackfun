...
const cC = `<form>\r
    <div class="form-group">\r
        <div class="inputGroup">\r
            ...
            <span class="inlineОтправить">\r
                <button type="submit" class="btn btn-default inlineОтправитьButton" type="button"><span class="inlineОтправитьText">Отправить</span></button>\r
            </span>\r
    ...
    to = Et.View.extend({
        ...
        events: {
            ...
            "click .inlineОтправитьButton": "onОтправитьClick",
...
`,
    ...
    MC = Et.View.extend({
        ...
        events: {
            "click #submitdrawing": "onChildviewButtonОтправить"
        },
        bindings: {
            ":el": {
                attributes: [{
            ...
            ".submitDrawing": {
                observe: ["hideОтправить", "actions", "strings"],
                visible: !0,
                updateView: !0,
                onGet(t) {
        ...
        autoОтправить() {
            this.sketchpadComponent.end(), this.sketchpadComponent.getLines().length > 0 && this.onChildviewButtonОтправить()
        },
        ...
        onChildviewChildviewButtonОтправить() {
            this.onChildviewButtonОтправить()
        },
        onChildviewButtonОтправить() {
...
`,
    ...
    NC = Et.View.extend({
        ...
        events: {
            "submit form": "onChildviewInputОтправить"
        },
        bindings: {
            ".enterSingleTextForm": {
                observe: "entry",
        ...
        initialize() {
            this.currentEntry = null, this.shouldОтправить = !1, this.promptComponent = new $i({
                model: new ot.Model({
                    text: "",
                    className: ""
                })
        ...
        onChildviewInputText(t) {
            let e = t.getValue();
            e.length > this.model.get("maxLength") && (e = e.substr(0, this.model.get("maxLength")), t.setValue(e)), this.shouldОтправить = e.length > 0, this.model.get("live") && (this.throttledSend || (this.throttledSend = at.throttle(() => {
        ...
        onChildviewInputОтправить() {
            ...
            t.length > this.model.get("maxLength") && (this.inputComponent.setValue(t.substr(0, this.model.get("maxLength"))), t = this.inputComponent.getValue(), e = this.inputComponent.getSanitizedValue()), this.shouldОтправить = !1;
            const n = {
                    action: "write",
                    entry: e
                },
            ...
            if (i !== "back" && i !== "censorConfirm" && i !== "activateContentId")
                ...
                didOpen: () => {
                    ...
                    a.collection.add({
                        ...
                        placeholder: "???-????",
                        inlineОтправить: !0,
                        inlineОтправитьText: this.model.get("strings").vip_episodes_submit || "Отправить",
...
`,
    ...
    YC = Et.View.extend({
        ...
        onBeforeDestroy() {
            this.model.get("type") === "multiple" && this.onChildviewChildviewButtonОтправить()
        },
        update() {
            this.promptComponent.model.clear({
                silent: !0
        ...
        onChildviewChildviewButtonChoose(t) {
            ...
            switch (this.choicesList.children.forEach(a => {
                ...
                case "multiple":
                    ...
                    return !1;
                case "repeating":
                    if (this.votesLeft === void 0 && (this.votesLeft = this.model.get("maxVotes")), this.delayОтправить || this.votesLeft <= 0 && this.model.get("isAudience")) return !1;
                    this.delayОтправить = !0, window.setTimeout(() => {
                        this.delayОтправить = !1
...
const xx = `<div id="controller" class="state-controller controller-content">\r
    ...
    Ox = Et.View.extend({
        ...
        update() {
            ...
            this.episodesList.collection.set(e), this.inputComponent.model.set("maxLength", this.model.get("maxContentLength")), this.inputComponent.model.set("inlineОтправитьText", this.model.get("strings").button_add), this.titleInputComponent.model.set("maxLength", this.model.get("maxTitleLength")), this.titleInputComponent.model.set("inlineОтправитьText", this.model.get("strings").create_new_button), this.promptsList.collection.set(this.model.get("prompts").map(n => {
                const i = at.extend({}, n);
        ...
        <div id="state-make-many-choices" class="pt-page-off game-page textured-background">\r
            ...
            <div class="container">\r
                ...
                <div id="make-many-choices-submit-button-container" style="width:100%; display:table;" >\r
                    <button type="button" id="make-many-choices-submit-button" class="light-text button-game button-xlarge pure-button pure-input-1">Отправить</button>\r
                </div>\r
            </div>\r
        </div>\r
        \r
            ...
            <div>\r
                ...
                <div id="enter-single-text-input-container" class="container">\r
                    <form class="pure-form" id="enter-single-text-field">\r
                        ...
                        <button type="submit" id="enter-single-text-submit" class="light-text button-game button-large pure-button capitalize right"><i class="fas fa-paper-plane"></i>&nbsp;&nbsp;Отправить</button>\r
                    </form>\r
                </div>\r
            </div>\r
        </div>\r
...
\r
            ...
            <form id="enter-single-drawing-submit-container" class="pure-form container">\r
                <button type="submit" id="enter-single-drawing-submit" class="light-text submit-drawing button-game button-large pure-button pure-input-1" style="margin-top: 0px;"><i class="fas fa-paper-plane"></i>&nbsp;&nbsp;Отправить</button>\r
            </form>\r
        </div>\r
        \r
...
const Gx = VC.extend({
    ...
    async update() {
        if (this.model.get("room") && this.model.get("room").gameResults) {
            ...
            if (this.client.isRole("player")) {
                ...
                const d = e ? e.state : "";
                if (a) {
                    if (n += `<p>${a.wonGame?"ПОЗДРАВЛЯЮ":"КАКОЙ УЖАС!"}</p>`, d === "MakeSingleChoice")
                    ...
                    i += `<p>${a.wonGame?"Ты выжил в игре!":"Ты не выжил в игре"}</p>`, i += "<BR>", i += `<p>Твой итоговый счёт: $${a.score}</p>`
                }
                ...
                t.audience && (n += `<p>${t.audience.survived?"ПОЗДРАВЛЯЮ":"КАКОЙ УЖАС!"}</p>`, i += `<p>${t.audience.survived?"Зрители ВЫЖИЛИ!":"Зрители СДОХЛИ, и ты тоже!"}</p>`, i += "<BR>", i += `<p>${t.audience.count} ${t.audience.count===1?" зритель набрал":" зрителей набрали"}  : $${t.audience.score}</p>`, i += "<BR>", i += `<p>Твоей целью были : $${a}</p>`);
    ...
    updatePlayer() {
        ...
        if (n === "GameLocked") {
            kt.show(Error("В моём подвале уже достаточно будущих мертвецов. Зайди позже."), {
                willClose: () => {
                    window.location.reload(!0)
                }
            });
        ...
        if (i === "Lobby") {
            if (this.hideLobbyButtons(), !e.isAllowedToStartGame) {
                se("#lobby-text").html("Откинься на спинку кресла и отдохни!"), this.showScreen("#state-lobby"), this.setupWithDollInfo(e.dollInfo);
                return
            }
            const g = t.lobbyState;
            g === "WaitingForMore" ? se("#lobby-text").html("Ожидаем других игроков...") : g === "CanStart" ? (se("#lobby-text").html("Нажми эту кнопку, когда все присоединились"), se("#button-startgame").show()) : g === "Countdown" ? (se("#lobby-text").html("Нажми эту кнопку, чтобы отменить запуск"), se("#button-stopcountdown").show()) : g === "PostGame" && (se("#lobby-text").html("Что выберешь?"), se(".button-endbuttons").show()), this.showScreen("#state-lobby")
        } else if (i === "Gameplay")
            if (n === "MakeSingleChoice") {
                ...
                } else se("#make-single-choice-text").html("<p>Спасибо.</p>"), se("#make-single-choice-choices").html("");
            ...
            if (e.entry) this.showScreen("#state-logo");
                ...
                se("#enter-single-drawing-text-container").html(`<p>Пожалуйста, нарисуй:<br>${e.text}</p>`), this.showScreen("#state-enter-single-drawing");
            ...
            let J = function(G, Q) {
                ...
                q = function() {
                    ...
                    if(e.showProgress){
                        ...
                        let u = "АВСДЕБЖНЬЧКГМИОРЦЯЗТПЛШХУЫ";
                        let G = "";
                        for(let Q = 0; Q < D.length; Q++){
                            let ch = e.grid[D[Q].r][D[Q].c];
                            G += (ch === "" || ch === undefined || a.indexOf(ch) === -1) ? ch : u[a.indexOf(ch)];
                        }
                        se("#grid-submit").text(`Отправить${G.length>0?` "${G}"`:""}`)
                    } else se("#grid-submit").text("Отправить");
    ...
    updateAudience() {
        ...
        if (a === "single") {
            ...
            se("#make-many-choices-text").html(`<p>${n}</p>`), se("#make-many-choices-sub-text").html("<p>Выбери элементы, которые относятся к этой категории.</p>");
            let d = "";
    ...
    chooseChoice(t) {
        ...
        return this.client.isRole("player") ? this.client.send("SendMessageToRoomOwner", {
            ...
            type: "vote",
            vote: e
        }), se("#make-single-choice-text").html("<p>Спасибо. Теперь сиди тихо и жди.</p>"), se("#make-single-choice-choices").html("")), se(".button-choice").removeClass("selected"), se(t.currentTarget).addClass("selected"), !1
    },
    chooseManyChoices(t) {
        const e = se(t.currentTarget).data("choice"),
    ...
    submitManyChoices() {
        if (this.client.isRole("player")) {
            ...
            this.client.sessionSend("vote", "Trivia Death Vote", {
                type: "vote",
                vote: n.join(",")
            }), se("#make-many-choices-text").html("<p>Спасибо.</p>"), se("#make-many-choices-sub-text").html(""), se("#make-many-choices-choices").html(""), se("#make-many-choices-submit-button-container").hide()
        }
    },
