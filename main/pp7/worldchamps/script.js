    ...
    static async loadBannerData(e) {
        try {
            const i = await (await fetch({
                ...
                BASE_URL: "https://bundles.jackbox.tv/main/pp7-worldchamps/",
                MODE: "production",
                DEV: !1,
                PROD: !0
            }.TV_BANNERS_URL)).json();
...
class _t {
    ...
    static async showAlert(e) {
        ...
        return e.customClass = {
            ...n,
            popup: "jbgModal"
        }, e.titleText = e.titleText || "Ошибка", Yt.fire(e)
    }
    static async showError(e) {
        const n = new URL("main/pp7/worldchamps/assets/8cdd50e7.png", self.location).href,
            i = e.customClass || {};
        return e.customClass = {
            ...i,
            popup: "jbgModal"
        }, e.titleText = e.titleText || "Ошибка", n && (e.imageUrl = n), Yt.fire(e)
    }
...
Ct.View.extend({
    ...
    showTwitchBroadcasterDialog(t) {
        ...
        e += "<div class='success'>Вы успешно подключены к Jackbox Audience Kit Twitch Extension.</div>", this.lacksAudience ? e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>" : this.client.roomInfo.audienceEnabled || (e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>"), _t.show("custom", {
            html: e,
            position: "bottom",
            timer: t,
...
const Bx = Ct.View.extend({
    ...
    showTwitchBroadcasterDialog(t) {
        ...
        e += `
            ...
            </div>`, this.lacksAudience ? e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>" : this.client.roomInfo.audienceEnabled || (e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>"), _t.show("custom", {
            html: e,
            position: "bottom",
            timer: t,
            backdrop: "transparent",
...
const zx = Ct.View.extend({
    ...
    Hx = In.extend({
        defaults: He.extend({}, jn.prototype.model.defaults, {
            ...
            strings: He.extend({}, jn.prototype.model.defaults.strings, {
                name_modal_text: "Назови своего чемпиона:",
                name_modal_confirm: "OK",
                name_modal_cancel: "Отмена",
                name_error_empty: "Надо что-нибудь ввести!",
                ERROR_REJECTED_OBJECT: "Действие не разрешено! Нужно ввести что-то другое.",
                ERROR_REJECTED_TEXT: "Действие не разрешено! Нужно ввести что-то другое."
            })
        })
    }),
    Gx = Xs.extend({
    ...
    qx = jn.extend({
        ...
        nameCharacter() {
            ...
            Yt.close(), Yt.fire({
                ...
                cancelButtonText: this.model.get("strings").name_modal_cancel || "Отмена",
    ...
    Xx = In.extend({
        defaults: {
            ...
            announcePrompt: !1,
            strings: {
                survey_finished: "Спасибо за ответ. Вы исполнили свой гражданский долг!"
            }
        }
    }),
    Kx = Ct.View.extend({
...
const tE = Bx.extend({
    sessionModulePrefix: "worldchamps",
    getGameLayout(t) {
        switch (t) {
            case "Draw":
