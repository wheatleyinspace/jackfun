...
xt.View.extend({
    ...
    showTwitchBroadcasterDialog(t) {
        ...
        e += "<div class='success'>Вы успешно подключены к Jackbox Audience Kit Twitch Extension.</div>", this.lacksAudience ? e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>" : this.client.roomInfo.audienceEnabled || (e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>"), bt.show("custom", {
            html: e,
            position: "bottom",
            timer: t,
...
const $x = xt.View.extend({
    ...
    showTwitchBroadcasterDialog(t) {
        ...
        e += `
            ...
            </div>`, this.lacksAudience ? e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>" : this.client.roomInfo.audienceEnabled || (e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>"), bt.show("custom", {
            html: e,
            position: "bottom",
            timer: t,
            backdrop: "transparent",
...
const Gx = xt.View.extend({
    ...
    Hx = `<div>Вы действительно хотите скрыть ответ <span class="censorMenuTitle">title</span></div>
<div class="btn-toolbar choices">
    <div class="button-group btn-group btn-block">
        <button type="button" style="" class="button btn btn-lg censorRound">Скрыть этот ответ</button>
    </div>
    <div class="button-group btn-group btn-block">
        <button type="button" style="" class="button btn btn-lg censorGame">Скрыть все ответы игрока</button>
    </div>
    <div class="button-group btn-group btn-block">
        <button type="button" style="" class="button btn btn-lg cancel">ОТМЕНА</button>
    </div>
</div>`,
    qx = xt.View.extend({
    ...
    yh = ih.extend({
        ...
        buttonClick(t) {
            ...
            return this.model.get("isPlayer") ? i === "censor" ? bt.show("custom", {
                ...
                prompt: {
                    text: `Спасибо. Ваш выбор: ${this.model.get("choices")[n].text}`,
                    html: null
                },
                choices: []
            })), !1
    ...
    <div class="inputGroup">
        ...
        <div class="enterText">
            ...
            <span class="inlineSubmit">
                <div class="col-xs-6">
                    <button type="button" class="btn btn-block lieForMe" type="button">Спасалка</button>
                </div>
                <div class="col-xs-6">
                    <button type="submit" class="btn btn-block submitButton" type="button">Отправить</button>
                </div>
            </span>
        </div>
    </div>
...
<div class="entryDone">
    Спасибо. Вы ввели: <span class="entryText">entryText</span>
</div>
</form>`,
    Jx = us.extend({
    ...
    Ec = rs.extend({
        ...
        update() {
            rs.prototype.update.apply(this), this.model.get("requireLie") === !0 && this.inputComponent.model.set("answerDelim", "Напиши одну ложь о себе!"), this.inputComponent.model.set("doubleInput", this.model.get("requireLie") || this.model.get("doubleInput")), this.inputComponent.model.set("canUseSuggestions", this.model.get("canUseSuggestions")), this.inputComponent.model.set("usedSuggestion", this.model.get("usedSuggestion")), this.inputComponent.model.set("question", this.model.get("question")), this.model.get("entry") && this.model.get("entry").length > 0 ? this.buttonsCollection.reset() : this.model.get("suggestions") && this.buttonsCollection.set(this.model.get("suggestions").map(t => ({
                block: !0,
                text: t
            })))
        },
        ...
        onChildviewInputSubmit() {
            ...
            if (this.inputComponent.model.set("error", ""), typeof t == "object") {
                if (t[0] = t[0].replace(/\s\s+/g, " ").trim(), t[1] = t[1].replace(/\s\s+/g, " ").trim(), e.inputs = t, t[0] === "" || t[1] === "") return this.inputComponent.model.set("error", "Надо дать оба ответа!"), !1
            } else if (t = t.replace(/\s\s+/g, " ").trim(), e.entry = t, e.entry.length === 0) return this.inputComponent.model.set("error", "Надо что-то написать!"), !1;
    ...
    Zx = Ys.extend({
        ...
        update() {
            Ys.prototype.update.apply(this, []), !this.model.get("playerCanStartGame") && this.model.get("type") === "TMI" && this.titleComponent.model.set("text", "СОВЕТ: в этой игре используй своё настоящее имя!")
        }
    }),
    eE = hs.extend({
        bindings: Object.assign(hs.prototype.bindings, {
            "#playername": {
                ...
                onGet(t) {
                    let e = "зритель";
                    return t ? (t.displayName ? e = t.displayName : t.name && (e = t.name), e.toUpperCase()) : "зритель"
                }
            },
            "#playericon": {
                attributes: [{
...
const tE = $x.extend({
    ...
    parseBlob(t) {
        return t.playerInfo = {
        ...
        }, t.state === "EndShortie" && (t.state = "Logo"), t.state === "Lobby" && (this.client.isRole("player") || (t.state = "Logo"), t.playerInfo = t.playerInfo || {}, t.playerInfo.username = t.displayName || t.playerName || "зритель", t.playerInfo.username = t.playerInfo.username.toUpperCase(), t.playerIsVIP = t.isAllowedToStartGame, t.playerCanStartGame = t.isAllowedToStartGame, t.playerCanCensor = t.canCensor, t.gameCanStart = ["CanStart", "Countdown", "PostGame"].indexOf(t.lobbyState) !== -1, t.gameIsStarting = t.lobbyState === "Countdown", t.gameFinished = t.lobbyState === "PostGame", t.type === "TMI" && this.client.isRole("player") && !t.artifact && (t.canChangeName = !0)), t.state === "Logo" && (t.classes = [t.type], t.actions = []), ["EnterText", "EnterTruth"].indexOf(t.state) !== -1 && (t.prompt = {
            html: t.question
            ...
            text: `${t.choosingPlayerName} выбирает категорию...`
