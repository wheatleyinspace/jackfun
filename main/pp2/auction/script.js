...
const jx = Et.View.extend({
    ...
    <link rel="stylesheet" type="text/css" href="https://fonts.googleapis.com/css?family=Podkova:400,700&subset=cyrillic">\r
    \r
        ...
        <div id="state-lobby" class="pt-page-off pushed-down-page auction-page">\r
            <div class="container">\r
                ...
                <form class="pure-form">                    \r
                    <button type="button" id="auction-startgame" class="button-auction button-xlarge pure-button pure-input-1">ВСЕ ЗДЕСЬ</button>\r
                    <button type="button" id="auction-stopcountdown" class="button-auction button-xlarge pure-button pure-input-1">ОТМЕНА</button>  \r
                </form>\r
            </div>\r
        </div>\r
\r
...
\r
            ...
            <form class="pure-form container">\r
                <button type="submit" id="auction-submitdrawing" class="submit-drawing button-auction button-large pure-button pure-input-1" style="margin-top: 0px;"><i class="fas fa-paper-plane"></i>&nbsp;&nbsp;отправить</button>\r
        ...
        <div id="state-done-drawing" class="pt-page-off pushed-down-page auction-page">\r
            <br/><span class="big-text">Спасибо за рисунки!</span><br/>\r
        </div>\r
        \r
        <div id="state-auction" class="pt-page-off auction-page">\r
            ...
            <div id="auction-skip-content"><button type="button" id="auction-skip-button" class="button-auction button-large pure-button pure-input-1">Пропустить</button></div>\r
        ...
        <div id="state-post-game" class="pt-page-off pushed-down-page auction-page">\r
            <div class="container">\r
                ...
                <form class="pure-form">                    \r
                    <button type="button" id="auction-sameplayers" class="button-auction  button-xlarge pure-button pure-input-1 auction-endbuttons">ТЕ ЖЕ ИГРОКИ</button>\r
                    <button type="button" id="auction-newplayers" class="button-auction  button-xlarge pure-button pure-input-1 auction-endbuttons">НОВЫЕ ИГРОКИ</button>    \r
                </form>\r
            </div>\r
            \r
        </div>\r
...
const Xx = cc.extend({
    ...
    async update() {
        ...
        if (n === "GameLocked") kt.show(Error("Игра уже началась. Дождитесь начала следующей."), {
            willClose: () => {
                window.location.reload(!0)
            }
        });
            ...
            if (this.hideLobbyButtons(), !e.isAllowedToStartGame) {
                _e("#auction-lobby-text").html("Откинься на спинку кресла и отдохни!"), this.showScreen("#state-lobby");
                return
            }
            const m = t.lobbyState;
            m === "WaitingForMore" ? _e("#auction-lobby-text").html("Ожидаем других игроков...") : m === "CanStart" ? (_e("#auction-lobby-text").html("Нажми эту кнопку, когда все присоединились"), _e("#auction-startgame").show()) : m === "Countdown" && (_e("#auction-lobby-text").html("Нажми эту кнопку, чтобы отменить запуск"), _e("#auction-stopcountdown").show()), this.showScreen("#state-lobby")
            ...
            this.titleThatImDrawing = e.title, _e("#page-auction #state-draw #title").html(`Пожалуйста, нарисуй: ${this.titleThatImDrawing.text}`), this.showScreen("#state-draw");
            ...
            let m = "";
            if (e.info && e.message)
                for (f = 0; f < e.info.length; f++) m += '<div class="auction-info-content text-content">', m += `<p>${e.info[f].title} стоит ${this.formatMoney(e.info[f].value)}</p>`, m += "</div>";
            ...
            if (e.screws !== void 0) {
                ...
                if (t.screwingPlayerId)
                    ...
                    else R = "<span class='big-text'><p>Другой игрок использует шуруп, спокойнее</p></span>";
                else t.bids && (R = `<button type="button" class="pure-input-1 button-large pure-button button-auction auction-screw-button"${V?"":" disabled"}>ШУРУП</button>`)
            } else R = "";
            _e("#auction-screw-content").html(R);
            let D = "";
            if (t.playerSignalledForBank !== void 0) {
                let V, X;
                t.playerSignalledForBank ? (V = !0, X = "БАНК ВЫЗВАН") : e.numLoans >= 3 ? (V = !0, X = "КРЕДИТ НЕДОСТУПЕН") : e.timesOpenedBank >= 3 ? (V = !0, X = "ПРЕКРАТИ ЗВОНИТЬ") : (V = !1, X = "ПОЗВОНИТЬ В БАНК"), D += `<button type="button" class="pure-input-1 button-large pure-button button-auction auction-open-bank-button"${V?" disabled":""}>${X}</button><br>`
            } else t.loanAmount !== void 0 && t.debtAmount !== void 0 && e.hasTakenOutLoanInCurrentBank !== void 0 && (e.numLoans >= 3 ? D += `<span class="big-text"><p>Вы уже трижды брали кредит! Сначала выплатите долг.</p></span>` : (D += `<span class="big-text"><p>Вы получите: ${this.formatMoney(t.loanAmount)}, затем вам придётся вернуть ${this.formatMoney(t.debtAmount)}</p></span>`, D += `<button type="button" class="pure-input-1 button-large pure-button button-auction auction-take-loan-button"${e.hasTakenOutLoanInCurrentBank?" disabled":""}>ВЗЯТЬ КРЕДИТ</button><br>`));
            ...
            if (this.hideLobbyButtons(), !e.isAllowedToMakeChoice) {
                _e("#auction-lobby-text").html("Откинься на спинку кресла и отдохни!"), this.showScreen("#state-post-game");
                return
            }
            _e("#auction-post-game-text").html("Что выберешь?"), _e(".auction-endbuttons").show(), this.showScreen("#state-post-game")
        }
    },
    hideLobbyButtons() {
    ...
    submitDrawing() {
        if (this.currentCanvas.isClean) return alert("Нужно что-то нарисовать!"), !1;
