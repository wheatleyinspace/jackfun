...
class It {
    ...
    static async showAlert(e) {
        ...
        return e.customClass = {
            ...n,
            popup: "jbgModal"
        }, e.titleText = e.titleText || "Ошибка", Tn.fire(e)
    }
    static async showError(e) {
        ...
        return e.customClass = {
            ...i,
            popup: "jbgModal"
        }, e.titleText = e.titleText || "Ошибка", n && (e.imageUrl = n), Tn.fire(e)
    }
...
const lc = Et.View.extend({
        ...
        showTwitchBroadcasterDialog(t) {
            ...
            e += "<div class='success'>Вы успешно подключены к Jackbox Audience Kit Twitch Extension.</div>", this.lacksAudience ? e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>" : this.client.roomInfo.audienceEnabled || (e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>"), It.show("custom", {
                html: e,
                position: "bottom",
                timer: t,
...
Et.View.extend({
    ...
    showTwitchBroadcasterDialog(t) {
        ...
        e += `
            ...
            </div>`, this.lacksAudience ? e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>" : this.client.roomInfo.audienceEnabled || (e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>"), It.show("custom", {
            html: e,
            position: "bottom",
            timer: t,
            backdrop: "transparent",
        ...
        <div id="state-lobby" class="pt-page-off earwax-page">\r
            <div class="container">\r
                ...
                <form class="pure-form">\r
                    <button type="button" id="earwax-startgame" class="button-earwax button-xlarge pure-button pure-input-1">ВСЕ ЗДЕСЬ</button>\r
                    <button type="button" id="earwax-stopcountdown" class="button-earwax  button-xlarge pure-button pure-input-1">ОТМЕНА</button>\r
                    <button type="button" id="earwax-sameplayers" class="button-earwax  button-xlarge pure-button pure-input-1 earwax-endbuttons">ТЕ ЖЕ ИГРОКИ</button>\r
                    <button type="button" id="earwax-newplayers" class="button-earwax  button-xlarge pure-button pure-input-1 earwax-endbuttons">НОВЫЕ ИГРОКИ</button>    \r
                </form>\r
            </div>\r
        </div>\r
\r
        <div id="state-intro" class="pt-page-off earwax-page">\r
            <div class="container">\r
                ...
                <form class="pure-form">\r
                    <button type="button" id="earwax-skipintro" class="button-earwax  button-xlarge pure-button pure-input-1">ПРОПУСТИТЬ ИНСТРУКЦИЮ >></button>\r
                </form>\r
            </div>\r
        </div>\r
\r
...
\r
		<div class="pt-page-off state-choosing earwax-page">\r
			<div class="container">\r
				<br /><span>ВЫБЕРИ ЗАДАНИЕ</span><br />\r
...
\r
		<div class="pt-page-off state-audience-join earwax-page">\r
			<br /><span>Ты в зрителях!<br />Ожидаем следующего задания.</span><br />\r
		</div>\r
\r
...
const vx = lc.extend({
    ...
    async updatePlayer() {
        ...
        if (this.currentAnswerQuestionId = -1, n === "RoomFull") {
            It.show(Error("Комната заполнена"), {
                willClose: () => {
                    window.location.reload(!0)
                }
            });
        ...
        if (n === "GameLocked") It.show(Error("Игра уже началась – подожди начала следующей."), {
            willClose: () => {
                window.location.reload(!0)
            }
        });
            ...
            if (this.hideLobbyButtons(), !e.isAllowedToStartGame) {
                this.$el.find("#earwax-lobby-text").html("Откинься на спинку кресла и отдохни!"), this.showScreen("#state-lobby");
                return
            }
            const o = t.lobbyState;
            o === "WaitingForMore" ? this.$el.find("#earwax-lobby-text").html("Ожидание новых игроков...") : o === "CanStart" ? (this.$el.find("#earwax-lobby-text").html("Ожидание новых игроков..."), this.$el.find("#earwax-startgame").show()) : o === "Countdown" ? (this.$el.find("#earwax-lobby-text").html("Отменить запуск игры?"), this.$el.find("#earwax-stopcountdown").show()) : o === "PostGame" && (this.$el.find("#earwax-lobby-text").html("Что выберешь?"), Le(".earwax-endbuttons").show()), this.showScreen("#state-lobby")
            ...
            } else this.$el.find("#notchoosing").html(`${t.choosingPlayerName} выбирает задание`), this.showScreen(".state-notchoosing");
            ...
            if (e.isSelecting) {
                ...
                f ? this.$el.find("#selectingsound").html(`Выбери первый звук:<br>${t.prompt}`) : this.$el.find("#selectingsound").html(`Выбери второй звук:<br>${t.prompt}`), Le(".button-fieldset").html(o), this.showScreen("#state-selectingsound"), this.clickEnabled = !0
            } else this.$el.find("#notselectingsound").html("Ожидание остальных игроков..."), this.showScreen("#state-notselectingsound");
        else if (i === "Gameplay_SoundSelection" && n === "Gameplay_SoundSelected") this.$el.find("#notselectingsound").html("Ожидаем выбора остальных игроков..."), this.showScreen("#state-notselectingsound");
            ...
            let o = "";
            if (e.isChoosing) {
                this.$el.find("#vote-text").html("ВЫБЕРИ ПОБЕДИТЕЛЯ");
                const f = t.choices || [];
                ...
                this.$el.find("#vote-text").html("ГОЛОСУЙ ЗА ЛУЧШЕГО");
                const f = t.choices || [];
    ...
    updateAudience() {
        const t = this.model.get("room"),
            e = t ? t.state : "";
        if (e === "Gameplay_Vote") {
            t.prompt ? this.$el.find("#state-vote #question-text").html(t.prompt.replace(/<BLANK>/gi, "________")) : this.$el.find("#state-vote #question-text").html(""), this.$el.find("#vote-text").html("ГОЛОСУЙ ЗА ЛУЧШЕГО");
            let n = "";
            const i = t.choices || [];
        ...
        } else e === "Gameplay_CategorySelection" || e === "Gameplay_SoundSelection" ? (Le(".state-audience-wait-text").html("Ты в зрителях! Ожидаем голосования."), this.showScreen("#state-audience-wait")) : e === "Gameplay_Logo" ? this.showScreen("#state-logo") : this.showScreen(".state-audience-join")
    },
    hideLobbyButtons() {
    ...
    like(t) {
        ...
        return this.client.isRole("player") ? this.client.send("SendMessageToRoomOwner", {
            ...
            type: "vote",
            vote: e
        }), this.$el.find("#vote-text").html("Спасибо, зритель!"), this.$el.find("#earwax-vote").html(""), this.showScreen("#state-vote")), !1
    },
    newGameSamePlayers() {
        return this.client.send("SendMessageToRoomOwner", {
            startGame: !0,
