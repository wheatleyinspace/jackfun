        ...
        <div id="state-lobby" class="pt-page-off quiplash-page">\r
            <div class="container">\r
                ...
                <form class="pure-form">                    \r
                    <button type="button" id="quiplash-startgame" class="button-quiplash button-xlarge pure-button pure-input-1">ВСЕ ЗДЕСЬ</button>\r
                    <button type="button" id="quiplash-stopcountdown" class="button-quiplash  button-xlarge pure-button pure-input-1">ОТМЕНА</button>\r
                    <button type="button" id="quiplash-sameplayers" class="button-quiplash  button-xlarge pure-button pure-input-1 quiplash-endbuttons">ТЕ ЖЕ ИГРОКИ</button>\r
                    <button type="button" id="quiplash-newplayers" class="button-quiplash  button-xlarge pure-button pure-input-1 quiplash-endbuttons">НОВЫЕ ИГРОКИ</button>    \r
                </form>\r
            </div>\r
        </div>\r
\r
...
\r
                ...
                <form class="pure-form" id="quiplash-answer-field">\r
                    <div class="pure-u-1">\r
                        <input id="quiplash-answer-input" name="quiplash-answer" class="pure-input-1 capitalize jbg-input quiplash-answer-input" type="text" maxlength="45" placeholder="ПИШИ ЗДЕСЬ" autocapitalize="off" autocorrect="off" autocomplete="off">\r
                    </div>\r
                    <button type="submit" id="quiplash-submit-answer" class="button-quiplash button-large pure-button capitalize right">\r
                        <i class="fas fa-paper-plane"></i>&nbsp;&nbsp;Отправить\r
                    </button>\r
        ...
        <div id="state-done-answering" class="pt-page-off quiplash-page">\r
            <br/><span>Спасибо за ваши ответы!</span><br/>\r
        </div>\r
        \r
...
const Hx = VC.extend({
    ...
    async update() {
        ...
        if (n === "GameLocked") kt.show(Error("Игра уже началась. Дождитесь начала следующей."), {
            ...
            if (this.hideLobbyButtons(), !e.isAllowedToStartGame) {
                Ce("#quiplash-lobby-text").html("Откинься на спинку кресла и отдохни!"), this.showScreen("#state-lobby");
                return
            }
            const v = t.lobbyState;
            v === "WaitingForMore" ? Ce("#quiplash-lobby-text").html("Ожидаем других игроков...") : v === "CanStart" ? (Ce("#quiplash-lobby-text").html("Нажми эту кнопку, когда все присоединились"), Ce("#quiplash-startgame").show()) : v === "Countdown" ? (Ce("#quiplash-lobby-text").html("Нажми эту кнопку, чтобы отменить запуск"), Ce("#quiplash-stopcountdown").show()) : v === "PostGame" && (Ce("#quiplash-lobby-text").html("Что выберешь?"), Ce(".quiplash-endbuttons").show()), this.showScreen("#state-lobby")
            ...
            if (n === "Gameplay_AnswerQuestion") {
                ...
                if (this.currentAnswerQuestionId !== e.question.id && (Ce("#quiplash-answer-input").val(""), Ce("#quiplash-answer-field").show(), Ce("#quiplash-submit-alert").hide(), Ce("#state-answer-question #question-text").html(e.question.prompt.replace(/<BLANK>/gi, "________"))), e.showError) {
                    ...
                    Ce("#quiplash-submit-alert").html("Кто-то ещё дал точно такой же ответ! Подумай ещё."), v.addClass("alert-info"), v.removeClass("alert-danger"), v.show()
                }
                this.currentAnswerQuestionId = e.question.id, this.showScreen("#state-answer-question")
            } else Ce(".state-answer-question-audience-text").html("Ты в зрителях! Ожидаем голосования."), this.showScreen("#state-answer-question-audience");
        else if (i === "Gameplay_Vote") {
            if (e.doneVoting) {
                Ce("#vote-text").html("Ожидание остальных игроков"), Ce("#quiplash-vote").html(""), this.showScreen("#state-vote");
                return
            }
            t.question ? Ce("#state-vote #question-text").html(t.question.prompt.replace(/<BLANK>/gi, "________")) : Ce("#state-vote #question-text").html("");
            let v = "";
            e.votesLeft > 1 ? v = " голоса" : e.votesLeft === 1 && (v = " голос"), Ce("#vote-text").html(e.votesLeft ? (v === " голос" ? "Остался ": "Осталось " ) + ` ${e.votesLeft}${v}` : "Что тебе нравится больше?");
            let S = "";
    ...
    submitAnswer() {
        ...
        if (t.length === 0) {
            ...
            return Ce("#quiplash-submit-alert").html("Надо что-нибудь ввести."), e.removeClass("alert-info"), e.addClass("alert-danger"), e.show(), !1
        }
        return this.client.send("SendMessageToRoomOwner", {
            answer: t,
    ...
    vote(t) {
        ...
        return this.client.isRole("player") ? this.client.send("SendMessageToRoomOwner", {
            ...
            type: "vote",
            vote: e
        }), Ce("#vote-text").html("Спасибо, зритель!"), Ce("#quiplash-vote").html(""), this.showScreen("#state-vote")), !1
    },
    newGameSamePlayers() {
        return this.client.send("SendMessageToRoomOwner", {
            startGame: !0,
