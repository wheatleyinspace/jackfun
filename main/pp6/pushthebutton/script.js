...
class xt {
    ...
    static async showAlert(e) {
        ...
        return e.customClass = {
            ...n,
            popup: "jbgModal"
        }, e.titleText = e.titleText || "Ошибка", In.fire(e)
    }
    static async showError(e) {
        ...
        return e.customClass = {
            ...i,
            popup: "jbgModal"
        }, e.titleText = e.titleText || "Ошибка", n && (e.imageUrl = n), In.fire(e)
    }
...
rt.View.extend({
    ...
    showTwitchBroadcasterDialog(t) {
        ...
        e += "<div class='success'>Вы успешно подключены к Jackbox Audience Kit Twitch Extension.</div>", this.lacksAudience ? e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>" : this.client.roomInfo.audienceEnabled || (e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>"), xt.show("custom", {
            html: e,
            position: "bottom",
            timer: t,
...
const Ii = rt.View.extend({
    ...
    showTwitchBroadcasterDialog(t) {
        ...
        e += `
            ...
            </div>`, this.lacksAudience ? e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>" : this.client.roomInfo.audienceEnabled || (e += "<div class='warning'>В ЭТОЙ ИГРЕ НЕТ ФУНКЦИИ ЗРИТЕЛЕЙ</div>"), xt.show("custom", {
            html: e,
            position: "bottom",
            timer: t,
            backdrop: "transparent",
...
const jx = rt.View.extend({
    ...
    <div class="captain-indicator off glitchable glitch-text">КАПИТАН</div>
</div>
`,
    zx = Na.extend({
...
<div class="the-button">
    <button class="the-button-circle circle glitchable glitch-button">КНОПКА</button>
</div>

<div class="hurry-button">
    <button class="hurry-button-circle circle glitchable glitch-hurry">БЫСТРЕЕ!</button>
</div>`,
    Ux = rt.View.extend({
        tagName: "div",
        ...
        startInterval() {
            this.intervalCount = 0, this.model.set("message", "УДЕРЖИВАЙТЕ, ЧТОБЫ ПОДТВЕРДИТЬ"), this.model.set("shake", "none"), this.interval = setInterval(this.onInterval.bind(this), 1e3)
        },
        onInterval() {
            if (this.intervalCount += 1, this.intervalCount === 1) {
                this.model.set("message", "ВЫ УВЕРЕНЫ?"), this.model.set("shake", "light");
                return
            }
            if (this.intervalCount === 2) {
                this.model.set("message", "ЭТО НЕ ШУТКИ!"), this.model.set("shake", "heavy");
                return
            }
            this.triggerMethod("client:message", {
...
<button type="button" class="button choice-button btn btn-lg glitchable glitch-text"></button>`,
    bl = ss.extend({
        ...
        bindings: Object.assign(ss.prototype.bindings, {
            ...
            ".count": {
                observe: "numTimesTested",
                onGet(t) {
                    return (t > 1 && t < 5) ? `Проверен(-а) ${t} раза` : `Проверен(-а) ${t} раз`
                },
                classes: {
                    hidden: {
                        observe: "numTimesTested",
    ...
    Wx = Ws.extend({
        ...
        createConfirmDelay() {
            this.model.get("choices")[0].html = "ПОДТВЕРДИТЬ ЧЕРЕЗ 5", this.model.get("choices")[0].disabled = !0;
            let t = 4,
                e = window.setInterval(() => {
                    t ? this.model.get("choices")[0].html = `ПОДТВЕРДИТЬ ЧЕРЕЗ ${t}` : (this.model.get("choices")[0].html = "ПОДТВЕРДИТЬ", this.model.get("choices")[0].disabled = !1, window.clearInterval(e), e = void 0), t -= 1, this.update()
                }, 1e3)
        }
    }),
    Xx = Va.extend({
    ...
    <div>
        <div class="button-group btn-group btn-block">
            ...
            <button type="button" class="submit button choice-button btn btn-lg glitchable glitch-text">Отправить</button>
        </div>
    </div>
</div>`,
    Jx = rt.View.extend({
...
`,
    ...
    <div class="modal">
        <div class="container">
            ...
            <p class="title in">Отчёт биосканера</p>
            <p class="prompt in">Вам решать, как поступить с этой информацией.</p>
            <p class="result in">
                <span class="username"></span> – это <span class="role"></span>
            </p>
            <p class="help in">ВАЖНО: Возможно, пришельцы будут лгать.</p>
        </div>
    </div>
</div>`,
    o1 = rt.View.extend({
        ...
        bindings: {
            ...
            ".role": {
                observe: "player",
                onGet: t => t.role.replace("Human","Человек").replace("Alien","Пришелец")
            }
        },
        initialize() {
            this.listenTo(this.model, "change", this.update, this)
...
<button class="hack-button glitchable glitch-element"></button>`,
    ...
    m1 = Ue.Model.extend({
        ...
        responsesText() {
            ...
            return t ? (t.length > 1 && t.length < 5) ? `Проверен(-а) ${t.length} раза` : `Проверен(-а) ${t.length} раз` : ""
        },
        captainTimeText() {
        ...
        hackMessage() {
            return this.isHacked() ? this.hackedBy() === this.get("playerName") ? "Взломан(-а) ВАМИ" : `Взломан(-а) игроком ${this.hackedBy()}` : null
        },
        hackState() {
...
<button class="censor-button"></button>`,
    ...
    C1 = '<p class="empty glitchable glitch-text glitch-element">НЕТ ОТВЕТОВ ДЛЯ ИССЛЕДОВАНИЯ</p>',
    x1 = rt.View.extend({
    ...
    S1 = rt.View.extend({
        ...
        bindings: {
            ...
            ".count": {
                observe: "responses",
                onGet(t) {
                    return t ? `${this.model.responsesText()} | ${this.model.captainTimeText()} Время Капитана` : ""
                }
            },
            ".hack-button": {
                observe: "playerInfo",
