		...
		<div class="pt-page-off state-lobby drawful-page">\r
			<div class="container">\r
				...
				<form class="pure-form">					\r
					<button type="button" id="drawful-startgame" class="button-drawful button-xlarge pure-button pure-input-1">ВСЕ ЗДЕСЬ</button>\r
					<button type="button" id="drawful-stopcountdown" class="button-drawful button-xlarge pure-button pure-input-1">ОТМЕНА</button>\r
					<button type="button" id="drawful-sameplayers" class="button-drawful button-xlarge pure-button pure-input-1 drawful-endbuttons">ТЕ ЖЕ ИГРОКИ</button>\r
					<button type="button" id="drawful-newplayers" class="button-drawful button-xlarge pure-button pure-input-1 drawful-endbuttons">НОВЫЕ ИГРОКИ</button>	\r
				</form>\r
			</div>\r
		</div>\r
\r
...
\r
		<div class="pt-page-off state-drawing-sent drawful-page">\r
			<div class="container">\r
				...
				<span class='drawful-text'>спасибо за рисунок!</span><br />\r
			</div>\r
		</div>\r
\r
		<div class="pt-page-off state-drawing-done drawful-page">\r
			<div class="container">\r
				<br /><span class='drawful-text'>время вышло!</span><br />\r
			</div>\r
		</div>		\r
\r
...
\r
				...
				<form class="pure-form" id="drawful-enterlie-field">\r
					<div class="pure-u-1">\r
						<input id="drawful-lie-input" name="drawful-lie" class="pure-input-1 jbg-input" type="text" maxlength="45" placeholder="придумай название" autocapitalize="off" autocorrect="off" autocomplete="off">\r
					</div>\r
					<div class="pure-u-1 right">\r
						<button type="submit" id="drawful-submitlie" class="button-drawful button-xlarge pure-button right"><i class="fas fa-paper-plane"></i>&nbsp;&nbsp;отправить</button>\r
...
\r
		<div class="pt-page-off state-liereceived drawful-page">\r
			<div class="container">\r
				<br /><span class='drawful-text'>название введено!<br />ожидаем остальных игроков.</span><br />\r
			</div>\r
		</div>\r
\r
...
\r
		<div class="pt-page-off state-draw drawful-page">\r
			<!-- <div class="container"> -->\r
				<span id="drawful-prompt" class='prompt drawful-text'>пожалуйста, нарисуй:</span><br />\r
				<span id="drawful-instructions" class='instructions drawful-text'>рисунок самого себя!</span><br />\r
\r
				<canvas class="sketchpad" width='240' height='300' style='background-color:white;'>\r
					К сожалению, ваш браузер не поддерживается.\r
				</canvas>\r
				<form class="pure-form container">\r
					<button type="submit" id="drawful-submitdrawing" class="submit-drawing button-drawful button-large pure-button pure-input-1" style="margin-top: 0px;"><i class="fas fa-paper-plane"></i>&nbsp;&nbsp;Отправить</button>\r
...
const Hx = lc.extend({
    ...
    lacksAudience: !0,
    authorMessages: ["ты нарисовал это. подумай о том, что ты сделал.", "ты нарисовал это. расслабься.", "ты нарисовал это. подожди, пока остальные поймут, что ты сделал.", "ты нарисовал это. отменить уже поздно.", "ты нарисовал это. вдохни поглубже.", "ты нарисовал это. мощно.", "ты нарисовал это. как ощущения?", "ты нарисовал это. это твоя вина.", "ты нарисовал это. это... хорошо?", "ты нарисовал это. все приветствуют тебя!", "ты нарисовал это. посмотрим, что будет дальше."],
    events: {
    ...
    async update() {
        ...
        if (t && t.hasPicture === !1) {
            ...
            this.enableLoadingButton("#drawful-submitdrawing", !0), this.showScreen(".state-draw"), fe(".drawful .state-draw .instructions").html("рисунок самого себя!");
        ...
        if (this.joiningInterfaceIsUp = !1, i && i === "Lobby" && n === i) {
            ...
            f === "WaitingForMore" ? fe("#drawful-lobby-text").html("Ожидаем других игроков...") : f === "CanStart" ? (fe("#drawful-lobby-text").html("Нажми эту кнопку, когда все присоединились"), fe("#drawful-startgame").show()) : f === "Countdown" ? (fe("#drawful-lobby-text").html("Нажми эту кнопку, чтобы отменить запуск"), fe("#drawful-stopcountdown").show()) : f === "PostGame" && (fe("#drawful-lobby-text").html("Что выберешь?"), fe(".drawful-endbuttons").show()), this.showScreen(".state-lobby")
            ...
            if (this.activeScreen !== ".state-enterlie" && (t.isAuthor ? (fe("#drawful-lie-input").hide(), fe("#drawful-enterlie-field").hide(), fe("#drawful-submit-alert").hide(), fe("#drawful-submitlie").hide(), fe(".state-enterlie .author-text").html(this.authorMessages[Math.floor(Math.random() * this.authorMessages.length)])) : (fe(".state-enterlie .author-text").html(""), fe("#drawful-lie-input").val(""), fe("#drawful-lie-input").show(), fe("#drawful-enterlie-field").show(), fe("#drawful-submitlie").show(), fe("#drawful-submit-alert").hide())), t.showError) {
                ...
                fe("#drawful-submit-alert").html("ты близок к правильному названию или ответу другого игрока! введи другой ответ!"), f.addClass("alert-info"), f.removeClass("alert-danger"), f.show()
            }
            ...
            if (t.choosingDone || t.chosen != null || t.isAuthor) {
                t.isAuthor ? fe("#chooselikes-choice").html("") : fe("#chooselikes-choice").html(t.chosen ? `твой выбор: ${t.chosen}` : "ты ничего не выбрал."), fe("#chooselikes-text").html("поставь бонусные лайки");
                ...
                fe("#chooselie-text").html("каково настоящее название?");
    ...
    submitLie() {
        ...
        if (t = t.replace(/\s\s+/g, " ").trim(), t.length === 0) {
            ...
            return fe("#drawful-submit-alert").html("введи свой ответ!"), e.removeClass("alert-info"), e.addClass("alert-danger"), e.show(), !1
        }
