...
const $x = Et.View.extend({
    ...
    <div id="game" class="game pt-pageholder">    
		<div class="pt-page-off state-lobby fibbage-page">
			<div class="container">
				...
				<form class="pure-form">					
					<button type="button" id="fibbage-startgame" class="button-fibbage button-xlarge pure-button pure-input-1">ВСЕ ЗДЕСЬ</button>
					<button type="button" id="fibbage-stopcountdown" class="button-fibbage  button-xlarge pure-button pure-input-1">Отмена</button>
					<button type="button" id="fibbage-sameplayers" class="button-fibbage  button-xlarge pure-button pure-input-1 fibbage-endbuttons">ТЕ ЖЕ ИГРОКИ</button>
					<button type="button" id="fibbage-newplayers" class="button-fibbage  button-xlarge pure-button pure-input-1 fibbage-endbuttons">НОВЫЕ ИГРОКИ</button>	
				</form>
			</div>
		</div>
		...
		<div class="pt-page-off state-choosing fibbage-page">
			<div class="container">
				<br /><span>Ты выбираешь категорию!</span><br />
		...
		<div class="pt-page-off state-enterlie fibbage-page">
			<div class="container">
				...
				<form class="pure-form" id="fibbage-enterlie-field">
					<div class="pure-u-1">
						<input id="fibbage-lie-input" name="fibbage-lie" class="pure-input-1 capitalize jbg-input fibbage-lie-input" type="text" maxlength="45" placeholder="ВВЕДИ ЛОЖЬ" autocapitalize="off" autocorrect="off" autocomplete="off">
					</div>
					<button type="button" id="fibbage-lieforme" class="button-fibbage button-large pure-button capitalize left">ПОДСКАЗКА<br/>(половина очков)</button>
					<button type="submit" id="fibbage-submitlie" class="button-fibbage button-large pure-button capitalize right"><i class="fas fa-paper-plane"></i>&nbsp;&nbsp;ОТПРАВИТЬ</button>
		...
		<div class="pt-page-off state-liereceived fibbage-page">
			<br /><span>ЛОЖЬ ВВЕДЕНА!<br />Ожидаем остальных игроков.</span><br />
		</div>

		<div class="pt-page-off state-lyingdone fibbage-page">
			<br /><span>ВСЕ ОТВЕТЫ ПРИНЯТЫ!</span><br />
		</div>		
...
const zx = VC.extend({
    ...
    async update() {
        ...
        if (i && i.split("_")[0] === "Lobby" && n === i) {
            ...
            const f = i.split("_")[1];
            f === "WaitingForMore" ? we("#fibbage-lobby-text").html("Ожидаем других игроков...") : f === "CanStart" ? (we("#fibbage-lobby-text").html("Нажми эту кнопку, когда все присоединились"), we("#fibbage-startgame").show()) : f === "Countdown" ? (we("#fibbage-lobby-text").html("Нажми эту кнопку, чтобы отменить запуск"), we("#fibbage-stopcountdown").show()) : f === "PostGame" && (we("#fibbage-lobby-text").html("Что выберешь?"), we(".fibbage-endbuttons").show()), this.showScreen(".state-lobby")
        } else if (i === "Gameplay_Logo") this.showScreen(".state-nothing");
        else if (i === "Gameplay_Round") we(".round-text").html(`РАУНД  ${e.round}`), this.showScreen(".state-round");
            ...
            } else we("#notchoosing").html(`${Ut.safeText(e.choosingPlayerName)} выбирает категорию`), this.showScreen(".state-notchoosing");
            ...
            if (n === "Gameplay_EnterLie") {
                ...
                if (t.showError) {
                    ...
                    we("#fibbage-submit-alert").html("Ты ввёл правду! Придумай ложь!"), f.addClass("alert-info"), f.removeClass("alert-danger"), f.show()
                }
            ...
            if (t.choosingDone || t.chosen !== void 0) {
                we("#chooselikes-choice").html(t.chosen ? `Твой выбор: ${Ut.safeText(t.chosen)}` : "Выбор не был сделан"), we("#chooselikes-text").html("Присуди бонусные лайки");
                let v = "";
                ...
                we("#chooselie-text").html("Найди правду!");
                let v = "";
    ...
    submitLie() {
        ...
        if (t = t.replace(/\s\s+/g, " ").trim(), t.length === 0) {
            ...
            return we("#fibbage-submit-alert").html("Нужно что-то написать! Используй подсказку, если нужна помощь."), e.removeClass("alert-info"), e.addClass("alert-danger"), e.show(), !1
        }
        return this.client.send("SendMessageToRoomOwner", {
            lieEntered: t,
            usedSuggestion: !1
